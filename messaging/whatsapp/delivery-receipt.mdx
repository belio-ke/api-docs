...
| `phone`          | string | Yes            | Recipient phone number in E.164 format without the leading `+`.                                                                                                               |
...
<Warning>
    When provided, both fields are required. An invalid `receiptRequest` causes the entire request to be rejected with
