...
Example:
```json
{ "type": "BodyNamedParameter", "name": "name", "value": "Alice" }
```
...
Example:
```json
{ "type": "BodyPositionalParameter", "position": 1, "value": "Alice" }
```
...
* URL, copy, quick reply, voice call (WhatsApp in-app voice call), OTP, and coupon buttons
* Limited-time offers
* Carousel cards
...
|------------------------------|-------------------------------------|
| `BodyNamedParameter`         | Named body placeholders             |
| `BodyPositionalParameter`    | Positional body placeholders        |
| `HeaderNamedParameter`       | Named text header placeholders      |
| `HeaderPositionalParameter`  | Positional text header placeholders |
| `MediaTemplateParameter`     | Media headers                       |
| `LocationTemplateParameter`  | Location headers                    |
| `CopyParameter`              | Copy buttons                        |
| `TimeToLiveParameter`        | Time-limited templates              |
| `VoiceCallPayloadParameter`  | Voice call templates                |
| `OtpParameter`               | OTP templates                       |
| `UrlNamedParameter`          | Named URL placeholders              |
| `UrlPositionalParameter`     | Positional URL placeholders         |
| `CouponCodeParameter`        | Coupon buttons                      |
| `QuickReplyPayloadParameter` | Quick reply buttons                 |
| `LimitedTimeOfferParameter`  | Limited-time offers                 |
| `CarouselCardParameter`      | Carousel cards                      |
...
<Note>
    ...
    * Templates must be in a **sendable state** (must be approved and not disabled) to be used for sending messages
</Note>
