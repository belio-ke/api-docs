...
```json
{
  ...
  "requestId": "7tewbasen-1234-5678-90ab-cdef12345678",
  "phone": "+254123456789",
...
| `requestId`      | String | Original message request ID returned by the API when the message was submitted. Use this value to correlate the delivery receipt with the originating API request. |

### Correlating a Delivery Receipt

When a message is submitted, the API returns a unique request ID for that message. The same ID is included as requestId in the corresponding delivery receipt callback.
Clients can store the request ID from the API response and use it to match subsequent delivery receipts to the original message.
...
An optional object on sent requests that enables delivery status callbacks.
